#define _CRT_SECURE_NO_WARNINGS    // Prevent compile error result 
                                   // from `scanf` security warning
#include <stdio.h>
#include <string.h>
#include <fcntl.h>
#include <sys/stat.h>
...
int pick_a_number() {
  ...
  winning_number = (rand() % 20) + 1;  // Pick a number btw. 1 and 20.
...
int dealer_no_match() {
  ...
  for (i=0; i<16; i++) {
    numbers[i] = rand() % 100;  // Pick a number btw. 0 and 99.
    printf("%d\t", numbers[i]);
    if (i%8 == 7)
      printf("\n");
  }
...
int find_the_ace() {
  ...
  ace = rand()%3;  // P;ace the ace randomly.
  ...
  while(wager_one == -1) // Loop until walid wager is made.
  ...
  while (invalid_choice) {
    ...
    if (choice_two == 'c') {   // Change pick.
      i = invalid_choice = 0;  // Valid choide
